...
<div class="backdrop-blur-xl bg-slate-900/95 border border-white/10 flex flex-col h-full overflow-hidden rounded-2xl">
	<!-- Header -->
	<div class="border-b border-white/10 flex items-center justify-between p-4">
					...
					>
						<div class="flex items-start justify-between">
							<div class="flex flex-col gap-1">
								<h3 class="font-medium text-gray-200 text-sm">{report.name}</h3>
						...
						<div class="gap-2 grid grid-cols-4 text-xs">
							<div class="bg-black/20 flex flex-col items-center p-2 rounded-lg">
								<span class="font-mono text-green-400">{formatNumber(report.finalAtoms)}</span>
								<span class="text-gray-500 text-[10px]">Atoms</span>
							</div>
							<div class="bg-black/20 flex flex-col items-center p-2 rounded-lg">
								<span class="font-mono text-pink-400">{formatNumber(report.finalAPS)}/s</span>
								<span class="text-gray-500 text-[10px]">APS</span>
							</div>
							<div class="bg-black/20 flex flex-col items-center p-2 rounded-lg">
								<span class="font-mono text-amber-400">Lv.{report.finalLevel}</span>
								<span class="text-gray-500 text-[10px]">Level</span>
							</div>
							<div class="bg-black/20 flex flex-col items-center p-2 rounded-lg">
								<span class="font-mono text-cyan-400">{report.milestoneCount}</span>
								<span class="text-gray-500 text-[10px]">Milestones</span>
							</div>
						</div>

						<!-- Config Summary -->
						<div class="flex flex-wrap gap-2 text-[10px]">
							<span class="bg-white/5 px-2 py-1 rounded text-gray-400">
